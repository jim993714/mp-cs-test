...
import { checkIsOpenWard } from '@/api/activity/shopping-cashback';
import type { IBannerList } from '@/api/home/types';
import { getBannerList } from '@/api/ucenter';
...
const banners = ref<IBannerList[]>([]);
const isOpenWard = ref(false);
const clientTop = uni.getWindowInfo().windowHeight / 2;
const filterServiceList = computed(() => {
    return serviceList.filter(item => {
        return !isOpenWard.value ? item.routeName !== 'shoppingCashback' : true;
    });
});
...
async function getBanner() {
    const { list } = await getBannerList(studentShopId.value);
    banners.value = list;
}

async function getOpenWardStatus() {
    const status = await checkIsOpenWard();

    isOpenWard.value = status === 1;
}
